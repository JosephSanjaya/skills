...
class ProfileValidator:
    ...
    def _validate_baseline_profile(self) -> bool:
        ...
        critical_classes = ["Application", "MainActivity", "AppModule", "AppComponent"]
        ...
        if found_critical:
            ...
            print("  ⚠️  No critical classes found (Application, MainActivity, etc.)")
    ...
    def _validate_startup_profile(self) -> bool:
        ...
        if len(classes) > 500:
            ...
            print("  ✓ Profile size is reasonable")
    ...
    def _validate_apk_profiles(self) -> bool:
        ...
        try:
            with zipfile.ZipFile(self.release_apk, "r") as apk:
                ...
                if baseline_files:
                    print("  ✓ Baseline profile found:")
                    for f in baseline_files:
                        size = apk.getinfo(f).file_size
                        print(f"    - {f} ({size} bytes)")
                else:
                    print("  ✗ Baseline profile not found in APK")
                ...
                if startup_files:
                    print("  ✓ Startup profile found:")
                    for f in startup_files:
                        size = apk.getinfo(f).file_size
                        print(f"    - {f} ({size} bytes)")
                else:
                    print("  ⚠️  Startup profile not found in APK (optional)")
