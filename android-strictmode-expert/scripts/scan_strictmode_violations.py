            ...
            for idx, line in enumerate(lines):
                ...
                for name, rule in PATTERNS.items():
                    if re.search(rule["regex"], line):
                        ...
                        if "schemas.android.com" in line or "schemas.xmlsoap.org" in line:
                            continue
                        # Direct Boot requires context (skip general storage checks unless boot completion handler)
                        if name == "direct_boot_violation" and "LOCKED_BOOT_COMPLETED" not in "".join(lines):
                            continue
...
def main():
    ...
    for root, _, files in os.walk(target_dir):
        ...
        if any(part in root.split(os.sep) for part in [".git", "build", "node_modules", ".gradle"]):
            continue
        ...
        for file in files:
            if file.endswith((".kt", ".java", ".xml")):
                ...
                if violations:
                    print(f"\nFile: {file_path}")
                    for v in violations:
                        print(f"  [{v['severity']}] Line {v['line_num']}: {v['message']}")
