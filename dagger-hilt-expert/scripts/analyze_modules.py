...
class ModuleAnalyzer:
        ...
        for i, line in enumerate(self.lines, 1):
            if '@Module' in line:
                ...
                for j in range(i, min(i + 3, len(self.lines))):
                    if 'class ' in self.lines[j] and 'object ' not in self.lines[j] and 'interface ' not in self.lines[j]:
        ...
        for i, line in enumerate(self.lines, 1):
            if '@Provides' in line:
                ...
                for j in range(i, min(i + 5, len(self.lines))):
                    ...
                    if re.search(r'fun\s+\w+\([^)]+\):\s*\w+\s*=\s*\w+\s*$', self.lines[j]):
                        self.issues.append(Issue(
                            severity="warning",
                            line=j + 1,
    ...
    def check_singleton_usage(self):
        ...
        feature_keywords = ['onboarding', 'login', 'signup', 'checkout', 'payment', 'wizard', 'tutorial']
    ...
    def check_static_provides(self):
        ...
        in_object = False
        for i, line in enumerate(self.lines, 1):
            if 'object ' in line and '@Module' in self.content[:self.content.find(line)]:
                in_object = True
    ...
    def check_scope_consistency(self):
        ...
        if install_in:
            expected_scope = [k for k, v in scope_to_component.items() if v == install_in]
            for i, line in enumerate(self.lines, 1):
