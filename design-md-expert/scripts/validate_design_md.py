...
class DesignMDValidator:
    ...
    def _check_color_palette(self):
        ...
        if len(hex_colors) < 5:
            self.warnings.append(
                f"Only {len(hex_colors)} hex colors found. Expected at least 5 (primary, surface, text, border, semantic)"
            )

        # Check for semantic naming
        if not re.search(r"\bprimary\b", self.content, re.IGNORECASE):
            self.warnings.append(
                "No 'primary' color token found. Use semantic names, not descriptive (e.g., 'primary' not 'blue-500')"
            )

        # Check for contrast ratios
        if not re.search(r"\d+\.?\d*:\d+", self.content):
            self.warnings.append(
                "No contrast ratios found. Include contrast ratios for accessibility (e.g., '4.5:1')"
            )

        # Check for color roles/usage
        if not re.search(r"usage|role|use case", self.content, re.IGNORECASE):
            self.info.append(
                "Consider adding 'Usage' or 'Role' column to color tables to explain when to use each color"
            )
    ...
    def _check_typography(self):
        ...
        if len(sizes) < 4:
            self.warnings.append(
                f"Only {len(sizes)} font sizes found. Expected at least 4 (display, heading, body, caption)"
            )

        # Check for font weights
        weights = re.findall(
            r"\b(regular|medium|semibold|bold|\d00)\b", self.content, re.IGNORECASE
        )
        if len(weights) < 3:
            self.warnings.append(
                "Expected at least 3 font weights (Regular, Medium, Bold)"
            )

        # Check for line heights
        if not re.search(r"line.?height", self.content, re.IGNORECASE):
            self.warnings.append(
                "No line-height specifications found. Include line-height for each type scale"
            )

        # Check for type scale table
        if not re.search(r"\|.*size.*weight.*line", self.content, re.IGNORECASE):
            self.info.append(
                "Consider using a table format for typography scale (Size | Weight | Line Height | Use Case)"
            )
    ...
    def _check_spacing_system(self):
        ...
        if not re.search(r"spacing|space-|layout", self.content, re.IGNORECASE):
            self.warnings.append(
                "No spacing system found. Define spacing scale (e.g., 4px/8px base)"
            )
        ...
        if not re.search(r"grid|column", self.content, re.IGNORECASE):
            self.info.append(
                "Consider adding grid system specification (12-column, 16-column, etc.)"
            )
    ...
    def _check_component_rules(self):
        ...
        found = sum(
            1 for c in components if re.search(rf"\b{c}\b", self.content, re.IGNORECASE)
        )

        if found < 2:
            self.warnings.append(
                f"Only {found} common components found. Define at least: buttons, inputs, cards"
            )

        # Check for component states
        if not re.search(r"hover|focus|active|disabled", self.content, re.IGNORECASE):
            self.warnings.append(
                "No component states found. Define hover, focus, active, disabled states"
            )

        # Check for touch targets
        if not re.search(
            r"44px|48px.*touch|min.?height.*44", self.content, re.IGNORECASE
        ):
            self.info.append(
                "Consider specifying minimum touch target size (44px or 48px)"
            )
    ...
    def _check_donts_section(self):
        ...
        if not re.search(r"don'?t|never|avoid", self.content, re.IGNORECASE):
            self.errors.append(
                "No 'Don'ts' section found. This is CRITICAL for AI agents to avoid bad patterns"
            )
            return
        ...
        if nevers < 3:
            self.warnings.append(
                f"Only {nevers} negative constraint statements found. Add more anti-patterns (e.g., 'NEVER use pure black', 'NEVER mix >2 typefaces')"
            )
    ...
    def _check_accessibility(self):
        ...
        if found == 0:
            self.warnings.append(
                "No accessibility considerations found. Include WCAG 2.2 AA requirements"
            )
        elif found < 2:
            self.info.append(
                "Limited accessibility guidance. Consider adding more (contrast ratios, keyboard nav, ARIA labels)"
            )

    def _check_responsive_behavior(self):
        """Check for responsive/mobile specifications."""
        if not re.search(
            r"responsive|breakpoint|mobile|tablet|desktop", self.content, re.IGNORECASE
        ):
            self.warnings.append(
                "No responsive behavior specified. Define breakpoints and mobile adaptations"
            )

        # Check for breakpoint values
        breakpoints = re.findall(
            r"\b(\d+)px\b.*(?:breakpoint|mobile|tablet|desktop)",
            self.content,
            re.IGNORECASE,
        )
        if len(breakpoints) < 2:
            self.info.append(
                "Consider defining specific breakpoint values (e.g., 768px, 1024px)"
            )
