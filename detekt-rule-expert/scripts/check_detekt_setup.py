...
def check_gradle_file(gradle_path):
    if not os.path.exists(gradle_path):
        print_warn(f"build.gradle.kts not found at {gradle_path}. Skipping Gradle validation.")
        return True
    ...
    api_dep_match = re.search(r'(implementation|compileOnly|api)\s*\(\s*["\'](.*detekt-api.*)["\']\s*\)', content)
    if not api_dep_match:
        api_dep_match = re.search(r'["\'](.*detekt-api.*)["\']', content)
        if api_dep_match:
            print_warn("Found detekt-api dependency but could not determine scope (e.g., compileOnly).")
        else:
    ...
    if scope != "compileOnly":
        print_err(f"detekt-api dependency is declared as '{scope}' in {gradle_path}. It MUST be 'compileOnly' to avoid runtime classloader conflicts.")
        return False
    
    print_ok(f"detekt-api is correctly declared as compileOnly.")
    return True

def check_serviceloader(project_root):
    resources_path = os.path.join(project_root, "src", "main", "resources", "META-INF", "services")
...
def check_detekt_yml(yml_path):
    ...
    if validation_enabled and not excludes_configured:
        print_warn("config.validation is set to true in detekt.yml but no config.excludes is declared. Custom rule properties will fail build validation unless excluded.")
    elif validation_enabled and excludes_configured:
        print_ok("config.validation and config.excludes are both present in detekt.yml.")
    else:
        print_ok("config.validation is disabled or default. No exclude validation rules required.")

    return True

def main():
    if len(sys.argv) < 2:
        print("Usage: python3 check_detekt_setup.py <path_to_rules_project_root> [path_to_detekt_yml]")
        sys.exit(1)

    project_root = sys.argv[1]
    yml_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(project_root, "config", "detekt.yml")
