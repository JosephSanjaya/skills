...
def validate_design_md(file_path):
    ...
    for sec in required_sections:
        if not re.search(rf"^#+\s+{re.escape(sec)}", content, re.IGNORECASE | re.MULTILINE):
    ...
    color_block = re.findall(r"(?:^#+\s+Color\s+&\s+Roles.*?)(?=^#+|$)", content, re.IGNORECASE | re.MULTILINE | re.DOTALL)
    if color_block:
        ...
        if black_colors:
            log_warn("True black (#000000) found. Avoid true black for premium dark mode aesthetics (prefer dark navy/charcoal).")
            warnings += 1
    else:
    ...
    spacing_block = re.findall(r"(?:^#+\s+Layout\s+&\s+Spacing.*?)(?=^#+|$)", content, re.IGNORECASE | re.MULTILINE | re.DOTALL)
    if spacing_block:
        numbers = [int(n) for n in re.findall(r"\b\d+px\b", spacing_block[0])]
        off_grid = [n for n in numbers if n % 4 != 0]
        if off_grid:
            log_warn(f"Found off-grid spacing values (not multiples of 4px): {off_grid}")
            warnings += 1
        else:
    ...
    print("\n--- Validation Summary ---")
    if errors > 0:
        print(f"\033[91mValidation FAILED\033[0m with {errors} error(s) and {warnings} warning(s).")
        return False
    else:
