...
EMPTY_CLOSERS = [
    ...
    r"\boverall\b",
    r"\bin summary\b",
    r"\bby understanding these\b",
    r"\bby leveraging these\b",
]
...
def analyze_file(file_path: str):
    ...
    for cat, words in BANNED_WORDS.items():
        ...
        for word in words:
            ...
            pattern = (
                rf"\b{word}\b" if " " in word or cat == "phrases" else rf"\b{word}s?\b"
            )
    ...
    for idx, p in enumerate(paragraphs):
        ...
        for pattern in EMPTY_CLOSERS:
            if re.search(pattern, last_line):
                # Print snippet
                print(
                    f"  ✗ Paragraph {idx + 1} ends with empty closer matching '{pattern}':"
                )
    ...
    if semicolon_count > 0:
        print(
            f"  ✗ Found {semicolon_count} semicolons. Replace with periods/shorter sentences."
        )
    ...
    if space_colon_matches:
        print(
            f"  ✗ Found {len(space_colon_matches)} instances of space before colons (e.g., 'word :')."
        )
    ...
    if outside_matches:
        print(
            f"  ✗ Found {len(outside_matches)} instances of punctuation outside quotation marks (e.g., {', '.join(outside_matches[:5])})."
        )
    ...
    for idx, p in enumerate(paragraphs):
        ...
        for i in range(len(lengths) - 2):
            ...
            if (7 <= len1 <= 13) and (7 <= len2 <= 13) and (7 <= len3 <= 13):
                print(
                    f"  ✗ Paragraph {idx + 1} has 3 consecutive flat medium-length sentences: {len1}, {len2}, {len3} words."
                )
                rhythm_issues += 1
                issues_found += 1
                break
            # Or all long sentences (e.g., 22+ words)
                ...
                print(
                    f"  ✗ Paragraph {idx + 1} has 3 consecutive long sentences: {len1}, {len2}, {len3} words."
                )
                rhythm_issues += 1
                issues_found += 1
                break
